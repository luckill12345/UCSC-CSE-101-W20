...
// the lca of two ints, in subtree rooted at Node*. Works by recursion
Node *BST::lca(Node *current, int num1, int num2)
{
    if (current == NULL)
        return current;

    if (num1 > current->key && num2 > current->key)
        return lca(current->right, num1, num2);

    if (num1 < current->key && num2 < current->key)
        return lca(current->left, num1, num2);

    return current;
}
// getting the lca of two ints. Just a wrapper for real recursion function
Node *BST::lca(int num1, int num2)
{
